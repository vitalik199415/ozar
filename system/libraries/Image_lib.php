...
class CI_Image_lib {
	...
	var $wm_opacity			= 40;			// Image opacity: 1 - 100  Only works with image
	...
	function text_watermark()
	{
		...
		if ($this->wm_use_truetype)
		{
			...
			switch ($this->wm_vrt_alignment)
			{
				case "T":
					break;
				case "M":	$y_axis += floor(($this->orig_height/2)-($TH/2));
					break;
				case "B":	$y_axis += ($this->orig_height - $TH);
					break;
			}
			...
			switch ($this->wm_hor_alignment)
			{
				case "L": $x_axis += ($this->orig_width/40);
					break;
				case "R": $x_axis += floor($this->orig_width - $TW - ($this->orig_width/40));
					break;
				case "C": $x_axis += floor(($this->orig_width - $TW)/2);
					break;
			}
	...
	private function filter_opacity( &$img, $opacity ) //params: image resource id, opacity in percentage (eg. 80)
	{
		if( !isset( $opacity ) )
			{ return false; }
		$opacity /= 100;
