<?php
$EMAIL = 'ozar.company@gmail.com'; //   Ваш e-mail на который будут приходить заявки
...
switch (@$_POST['action']) { // рассматриваем варианты разных значений поля action
    ...
    break;
    default:
        sendmail("Заказ обратного звонка c ozar.company", render(
						'<div><b>Страна:</b> '.stripinput($_POST['country']).'</div>'.
                        '<div><b>Ф.И.О:</b> '.stripinput($_POST['name']).'</div>'.
                        '<div><b>Номер телефона:</b> '.stripinput($_POST['tel']).'</div>'.
						'<div><b>E-mail:</b> '.stripinput($_POST['email']).'</div>'.
						'<div><b>Skype:</b> '.stripinput($_POST['skype']).'</div>'.
						'<div><b>Примечание:</b> '.stripinput($_POST['descr']).'</div>'
        ),$EMAIL,$FROM,$REPLY);
        exit('200');
    break;
