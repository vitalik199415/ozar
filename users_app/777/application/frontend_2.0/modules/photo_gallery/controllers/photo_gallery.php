...
class Photo_gallery extends AG_Controller
{
	...
	public function index($data)
	{
		...
		if($this->id_users_modules)
		{
			...
			$url_menu = $this->variables->get_url_vars('menu_url');
			 if($url_menu == "events"){
			  $this->template->add_view_to_template('center_block', 'photo_gallery/albums_event'.$data[0], $data[1]+array('settings' => $this->settings));
			 }
			 else {
