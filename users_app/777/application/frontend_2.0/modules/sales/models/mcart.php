...
class Mcart extends AG_Model
{
	...
	public function edit_item()
	{
		...
		if(isset($post['rowid']) && isset($cart[$post['rowid']]) && isset($post['qty']) && ($post['qty'] = intval($post['qty'])) > 0)
		{
            ...
            if($data['qty'] < $product_price['min_qty'])
            {
                return array('success' => FALSE, 'message' => $this->lang->line('cart_error_min_qty_part1').$product_price['min_qty'].$this->lang->line('cart_error_min_qty_part2'), 'delay' => 5000);
            }
	...
	public function delete_item()
	{
		...
		if(isset($post['rowid']))
		{
			...
			if($cart[$post['rowid']])
			{
				...
				$mess = '';
				if($this->mdiscount_coupons->check_coupon_products($data) == 0) {
					$this->session->unset_data('coupon_code');
					$mess = $this->lang->line('d_c_no_products');
				}
				return array('success' => TRUE, 'message' => $this->lang->line('cart_success_delete_item').$mess);
			}
	...
	public function get_cart_products()
	{
		...
		if(count($cart)>0)
		{
			...
			//  пересчет стоимости товаров со скидкой
			$this->load->model('sales/mdiscounts');

			$discount = $this->mdiscounts->get_discount($data);

			if($discount['result'] == 2)
			{
				$disc_sum = $discount['discount'];
				$return_array = array(
					'cart_products' => $discount['products'],
					'cart_total_price' => '<s>'.number_format($this->cart->total() * $currency['rate'], 2, ',', ' ').'</s> '.number_format(($this->cart->total() - $disc_sum) * $currency['rate'], 2, ',', ' ').' '.$currency['name'],
					'cart_total_original' => $this->cart->total() * $currency['rate'],
					'currency_name' => $currency['name'],
					'discount' => $disc_sum
				);
			}
			else if($discount['result'] == 1)
			{
				$disc_sum = $discount['discount'];
				$return_array = array(
					'cart_products' => $data,
			...
			else
			{
				$return_array = array(
					'cart_products' => $data,
					'cart_total_price' => number_format($this->cart->total() * $currency['rate'], 2, ',', ' ').' '.$currency['name'],
					'cart_total_original' => $this->cart->total() * $currency['rate'],
					'currency_name' => $currency['name'],
					'discount' => 0
				);
