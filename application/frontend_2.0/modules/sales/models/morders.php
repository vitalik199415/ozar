...
class Morders extends AG_Model
{
	...
	public function save($id = FALSE)
	{
		...
		$letter_data += $this->save_order_products_data($order_data, $products_data_array['cart_products']);

		$this->db->trans_complete();
		if($this->db->trans_status())
		{
			$this->load->model('sales/mdiscount_coupons');
			$this->mdiscount_coupons->activate_code();
			$this->cart->destroy();
	...
	protected function save_order_data($products)
	{
		...
		$total_price = 0;
		$total_qty = 0;
		foreach($products['cart_products'] as $ms)
		{
			$total_price += $ms['products_prices']['original_total_price'];
			$total_qty += $ms['cart']['qty'] * $ms['products_prices']['real_qty'];
		}

		if($this->session->get_data('coupon_code')) {
			$promocode = $this->session->get_data('coupon_code');
		} else {
			$promocode = NULL;
		}

		$data = array('orders_number' => $max, 'total_qty' => $total_qty, 'subtotal' => $total_price, 'total' => $total_price, 'base_id_m_c_currency' => $base_currency['id_m_c_currency'], 'base_currency_name' => $base_currency['name'], 'id_m_c_currency' => $currency['ID'], 'currency_name' => $currency['name'], 'currency_rate' => $currency['rate'], self::ID_LANGS => $this->mlangs->id_langs, 'discount' => $products['discount'], 'discount_coupon_number' => $promocode);
		$no_save_data = array();
	...
	protected function save_order_products_data($order_data, $products)
	{
		...
		{
			if(isset($ms['products_prices']['special_total_price']) AND isset($ms['products_prices']['discount'])){
				$data = array('id_m_orders' => $order_data['id'], 'id_m_c_products' => $ms['products']['ID'], 'sku' => $ms['products']['sku'], 'name' => $ms['products']['name'], 'price' => $ms['products_prices']['cart_price'], 'discount' => $ms['products_prices']['discount'], 'qty' => $ms['cart']['qty'], 'real_qty' => $ms['products_prices']['real_qty'], 'subtotal' => $ms['products_prices']['original_total_price'], 'total' => $ms['products_prices']['original_total_price'], 'price_alias' => $ms['products_prices']['price_alias'], 'price_name' => $ms['products_prices']['price_name']);
			} else {
