...
<div class="shipping_address_block">
	...
	<legend><?=$this->lang->line('c_o_fieldset_shipping_address')?></legend>
		...
		<? } ?>
		<? if(@$order_data['sales_settings']['address_S_skype'] == 0 || @$order_data['sales_settings']['address_S_skype'] == 1){ $must_be = '' ?>
			<? if(@$order_data['sales_settings']['address_S_skype'] == 1) $must_be = ' (*)' ?>
			<div class="form_field order_field shipping_field"><label for="order_address[S][skype]">		<?=$this->lang->line('c_o_form_skype')?> (*)</label><input type="text" name="order_address[S][skype]" value="<?=@$order_data['customer_S_address_data']['skype']?>"><div class="clear_both"></div></div>
		<? } ?>
		<? if(@$order_data['sales_settings']['address_S_viber'] == 0 || @$order_data['sales_settings']['address_S_viber'] == 1){ $must_be = '' ?>
			<? if(@$order_data['sales_settings']['address_S_viber'] == 1) $must_be = ' (*)' ?>
			<div class="form_field order_field shipping_field"><label for="order_address[S][viber]">		<?=$this->lang->line('c_o_form_viber')?> (*)</label><input type="text" name="order_address[S][viber]" value="<?=@$order_data['customer_S_address_data']['viber']?>"><div class="clear_both"></div></div>
		<? } ?>
