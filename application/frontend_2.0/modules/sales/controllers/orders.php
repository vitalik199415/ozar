...
class Orders extends AG_Controller
{
	...
	public function save()
	{
		...
		if(isset($_POST) && count($_POST)>0)
		{
			...
			if($success)
			{
				...
				$this->create_registration_array();
	...
	private function create_registration_array() {
		$this->load->helper('string');
		$data = array();
		$post = $_POST;

		$nickname = explode('@', $post['order_address']['B']['address_email']);
		$password = random_string('alnum', 8);

		$data['customer']['name'] = $nickname[0];
		$data['customer']['email'] = $post['order_address']['B']['address_email'];
		$data['customer']['password'] = $password;
		$data['confirm_password'] = $password;

		$data['customer_address']['B']['name'] = $post['order_address']['B']['name'];
		$data['customer_address']['B']['country'] = $post['order_address']['B']['country'];
		$data['customer_address']['B']['city'] = $post['order_address']['B']['city'];
		$data['customer_address']['B']['address'] = $post['order_address']['B']['address'];
		$data['customer_address']['B']['telephone'] = $post['order_address']['B']['telephone'];
		$data['customer_address']['B']['address_email'] = $post['order_address']['B']['address_email'];

        $data['captcha'] = 1111;
        $this->session->set_data('captcha_keystring', 1111);
