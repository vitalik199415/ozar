...
class Search extends AG_Controller
{
	...
	public function start_search()
	{
		if($search_string = $this->input->post('search_string'))
		{
			...
			$hash = preg_replace('|(-+)$|ui','',trim($hash));
			redirect($this->router->build_url('search_data_lang', array('search_string' => $hash, 'lang' => $this->mlangs->lang_code, 'additional_params' => $this->variables->build_additional_url_params())) ,301);
		}
	}
