...
class Mcatalogue_search extends AG_Model
{
	...
	public function set_limit($count, $limit, $search_string)
	{
		...
		$add_param = $this->variables->build_additional_url_params();
		if($this->variables->get_url_vars('page') == 1) redirect($this->router->build_url('search_data_lang', array('search_string' => $search_string, 'lang' => $this->langs->id_langs, 'additional_params' => $add_param)) ,301);
		...
		{
			redirect($this->router->build_url('search_data_page_lang', array('page' => ceil($count/$limit), 'search_string' => $search_string, 'lang' => $this->langs->id_langs, 'additional_params' => $add_param)) ,301);
		}
		...
		if($page_count > 1)
		{
			if($page == 1)
			{
				$this->template->add_rel_next($this->router->build_url('search_data_page_lang', array('page' => $page + 1, 'search_string' => $search_string, 'lang' => $this->mlangs->lang_code, 'additional_params' => $add_param)));
			}
			else if($page < $page_count)
			{
				if($page > 2)
				{
					$this->template->add_rel_prev($this->router->build_url('search_data_page_lang', array('page' => $page - 1, 'search_string' => $search_string, 'lang' => $this->mlangs->lang_code, 'additional_params' => $add_param)));
				}
				else
				{
					$this->template->add_rel_prev($this->router->build_url('search_data_lang', array('search_string' => $search_string, 'lang' => $this->mlangs->lang_code, 'additional_params' => $add_param)));
				}
				$this->template->add_rel_next($this->router->build_url('search_data_page_lang', array('page' => $page + 1, 'search_string' => $search_string, 'lang' => $this->mlangs->lang_code, 'additional_params' => $add_param)));
			}
			else
	...
	public function build_pagination_array($pages_data, $search_string)
	{
		...
		if($pages_data && count($pages_data)>0)
		{
			...
			$pages_array = get_pages_array($pages_data[0], $pages_data[1], $pages_data[2], array('search_data_page_lang', array('search_string' => $search_string, 'page' => '', 'lang' => $this->mlangs->lang_code, 'additional_params' => $this->variables->build_additional_url_params())));
		}
		return $pages_array;
	}
}
