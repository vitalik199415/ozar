...
class Mcurrency extends AG_Model
{
	...
	public function get_current_currency()
	{
		...
		if($id_currency = $this->session->userdata('CURRENCY_ID'))
		{
			...
			$query = $this->db
				->select("B.`name`, B.`".self::ID_CUR."` AS ID, A.`rate`, A.`name`, A.`visible_rules`, A.`m_u_types`")
				->from("`".self::UCUR."` AS A")
				->join(	"`".self::CUR."` AS B",
	...
	public function get_currency_to_select()
	{
		...
		$query = $this->db->select("`".self::ID_CUR."` AS CUR_ID, `name`, `rate`, `visible_rules`, `m_u_types`")
				->from("`".self::UCUR."`")
