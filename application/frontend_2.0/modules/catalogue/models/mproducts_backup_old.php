...
class Mproducts extends AG_Model
{
	...
	protected function get_products_by_id_array($result = array(), $random = FALSE)
	{
		...
		$products = $array;
		$images_array = $this->build_products_short_image_array($products);
		
		if($random)
		{
			shuffle($products);
		}	
		return array('products' => $products, 'images' => $images_array);
	}
	...
	protected function build_products_sort_filtered_short_array($category_id)
	{
		...
		if($count > 0)
		{
			...
			$images_array = $this->build_products_short_image_array($products_array);
			if($pages_data)
			{
			...
			return array('products' => $products_array, 'images' => $images_array, 'pages' => $pages_array);
		}
		else
		{
			return array('products' => $products_array, 'images' => $images_array, 'pages' => $pages_array);
