...
class Index extends AG_Controller
{
	...
	{
        $this->session->set_data('back_url', 'http://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'].$_SERVER['QUERY_STRING']);
	...
	public function discount_coupons() {
		$this->session->set_data('back_url', 'http://'.$_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'].$_SERVER['QUERY_STRING']);
		echo modules::run('sales/discount_coupons/view_coupon');
	}
