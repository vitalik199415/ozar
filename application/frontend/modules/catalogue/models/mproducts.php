...
class Mproducts extends AG_Model
{
	...
	{
		$bestseller_id = array();
		$query = $this->get_BSN_products_query($count, $random);
		$query->where("`bestseller`", 1);
		$bestseller_products = $query->get()->result_array();
	...
	{
		$sale_id = array();
		$query = $this->get_BSN_products_query($count, $random, TRUE);
		$query->where("`sale`", 1);
		$sale_products = $query->get()->result_array();
	...
	protected function get_products_by_id($id, $random = FALSE, $in_stock = FALSE)
	{
		...
		if(is_array($id))
		{
			if(count($id)>0)
			{
				$query = $this->get_products_by_id_query($id, $in_stock);
				$_products = $this->get_products_by_id_array($query->get()->result_array(), $random);
	...
	protected function get_products_by_id_query($id, $in_stock = FALSE)
	{
		...
		if($wh_id = $this->mwarehouses->get_shop_wh_id())
		{
			...
			if($in_stock) $query->where("A.`in_stock`", 1);
			return $query;
		}
	...
	public function get_search_products_query($search, $categorie_id = FALSE)
	{
		$this->load->model('catalogue/mproducts_settings');
		$this->load->model('warehouse/mwarehouses');
		if($wh_id = $this->mwarehouses->get_shop_wh_id())
		{
			$query = $this->db
				->select("A.`".self::ID_PR."` AS ID, A.`sku`, A.`url_key`, A.`bestseller`, A.`new`, A.`sale`, B.`name`, B.`short_description`,
				IMG.`image`, IMG.`".self::ID_PR_ALB."` AS ALB_ID, IMG_DESC.`name` AS image_name, IMG_DESC.`title` AS image_title, IMG_DESC.`alt` AS image_alt,
				IF(A.`in_stock` > 0, IF(WHP.`qty` > 0, 1, 0), 0) AS `in_stock`")
				->from("`".self::PR."` AS A")
				->join(	"`".self::PR_DESC."` AS B",
	...
	public function get_categories_products_collection($categories_id)
	{
		$this->load->model('catalogue/mtypes');
		$select_array = $this->mtypes->get_categories_types_array();
		if($select_array)
		{
			$this->template->add_js('jquery.gbc_types', 'modules_js/catalogue/types');
			$this->template->add_view_to_template('types_block', 'catalogue/types/types', $select_array);
		}
		$products_array = $this->build_products_short_array($categories_id);
		$products_array['products_settings'] = $this->settings;
		
		return $products_array;
	}
	
	protected function build_products_short_array($categories_id)
	{
		$products_array = array();
		$pages_array = array();
		$count = $this->get_categories_products_count($categories_id);
		if($count > 0)
		{
			$page = 1;
			if($this->variables->get_url_vars('page') == 1) redirect($this->router->build_url('categorie_lang', array('categorie_url' => $this->variables->get_url_vars('categorie_url'), 'lang' => $this->langs->id_langs)) ,301);
			if((int) $this->variables->get_url_vars('page')>0)
			{
				$page = (int) $this->variables->get_url_vars('page');
			}
			
			$query = $this->get_categories_products_query($categories_id);
			$query = $this->mtypes->update_products_query($query);
			
			$pages_data = $this->set_limit($query, $count, $page);
			$products_array = $this->build_products_short_data($query->get()->result_array());
			
			if($pages_data && count($pages_data)>0)
			{
				$this->load->helper('pages');
				$pages_array = get_pages_array($pages_data[0], $pages_data[1], $pages_data[2], array('categorie_page_lang', array('categorie_url' => $this->variables->get_vars('categorie_url'), 'page' => '', 'lang' => $this->mlangs->lang_code)));
			}
	...
	protected function get_categories_products_count($categories_id)
	{
		$query = $this->db
				->select("COUNT(*) AS COUNT")
				->from("`".self::PR."` AS A")
				->join(	"`".self::PR_CAT."` AS PRCAT",
						"PRCAT.`".self::ID_CAT."` = '".$categories_id."' && PRCAT.`".self::ID_PR."` = A.`".self::ID_PR."`",
						"INNER")
	...
	protected function get_categories_products_query($categorie_id = FALSE)
	{
		if(!$categorie_id)
		{
			if(!$categorie_id = $this->variables->get_vars('categorie_id'))
			{
				return FALSE;
			}
		}
			...
			if(count($SHOP_WH) > 0)
			{
				...
				$query = $this->db
					...
					->join(	"`".self::PR_IMG_DESC."` AS IMG_DESC",
							"IMG_DESC.`".self::ID_PR_IMG."` = IMG.`".self::ID_PR_IMG."` && IMG_DESC.`".self::ID_LANGS."` = '".$this->mlangs->id_langs."'",
							"LEFT")
					->where("PRCAT.`".self::ID_CAT."`", $categorie_id)->where("PRCAT.`".self::ID_USERS."`", $this->id_users)->order_by("PRCAT.`new`", "DESC");
				
				if($this->settings['products_sort_type'] == 1) $query->order_by("PRCAT.`sort`", "DESC"); else $query->order_by("PRCAT.`sort`");
				return $query;
			}
		}
	...
	protected function build_products_short_prices_array($products_array)
	{
		...
		if($query = $this->get_short_products_prices_query($price_pr_array))
		{
			$products_array = $this->build_products_short_prices_data($query->get()->result_array(), $products_array);
		}
	...
	protected function set_limit($query, $count, $page = 1)
	{
		...
		{
			redirect($this->router->build_url('categories_page_lang', array('page' => ceil($count/$limit), 'categorie_url' => $this->variables->get_url_vars('categorie_url'), 'lang' => $this->langs->id_langs)) ,301);
		}
		...
		if($page_count > 1)
		{
			...
			if($page == 1)
			{
				$this->template->add_rel_next($this->router->build_url('categorie_page_lang', array('categorie_url' => $this->variables->get_vars('categorie_url'), 'page' => $page + 1, 'lang' => $this->mlangs->lang_code)));
			}
			else if($page < $page_count)
			{
				if($page > 2)
				{
					$this->template->add_rel_prev($this->router->build_url('categorie_page_lang', array('page' => $page - 1, 'categorie_url' => $this->variables->get_url_vars('categorie_url'), 'lang' => $this->mlangs->lang_code)));
				}
				else
				{
					$this->template->add_rel_prev($this->router->build_url('categorie_lang', array('categorie_url' => $this->variables->get_url_vars('categorie_url'), 'lang' => $this->mlangs->lang_code)));
				}
				$this->template->add_rel_next($this->router->build_url('categorie_page_lang', array('page' => $page + 1, 'categorie_url' => $this->variables->get_url_vars('categorie_url'), 'lang' => $this->mlangs->lang_code)));
			}
			else
			{
				if($page > 2)
	...
	{
		$product_array = array();
		$query = $this->get_product_detail_query($url);
	...
	protected function get_product_detail_query($url)
	{
		...
		if($wh_settings['wh_on'] == 1 && $wh_settings['wh_active'] == 1)
		{
			...
			if(count($SHOP_WH) > 0)
			{
				$wh_id = $SHOP_WH[self::ID_WH];
				$query = $this->db
					->select("A.`".self::ID_PR."` AS ID, A.`sku`, A.`url_key`, A.`status`, A.`bestseller`, A.`sale`, A.`new`, B.`name`, B.`full_description`, B.`short_description`, B.`seo_title`, B.`seo_description`, B.`seo_keywords`,
							IF(A.`in_stock` > 0, IF(WHP.`qty` > 0, 1, 0), 0) AS `in_stock`")
					->from("`".self::PR."` AS A")
					->join( "`".self::WH_PR."` AS WHP",
							"WHP.`".self::ID_PR."` = A.`".self::ID_PR."` && WHP.`".self::ID_WH."` = ".$wh_id,
		...
		$query = $this->db
				->select("A.`".self::ID_PR."` AS ID, A.`sku`, A.`url_key`, A.`status`, A.`in_stock`, A.`bestseller`, A.`sale`, A.`new`, B.`name`, B.`full_description`, B.`short_description`, B.`seo_title`, B.`seo_description`, B.`seo_keywords`")
				->from("`".self::PR."` AS A")
				->join(	"`".self::PR_DESC."` AS B",
						"B.`".self::ID_PR."` = A.`".self::ID_PR."` && B.`".self::ID_LANGS."` = '".$this->mlangs->id_langs."'",
						"left")
	...
	{
		if(($query = $this->get_product_detail_prices_query($product_id)) != FALSE)
		{
		...
		//$prices_array['prices'] = $this->generate_price_html($prices_array);
		return $prices_array;
	}
	...
	protected function build_product_detail_attributes_array($product_id, $selected_price = FALSE)
	{
		...
		//$attributes_array['attributes'] = $this->generate_attributes_html($attributes_array);
	...
	{
		$query = $this->db->select("A.`".self::ID_CAT."`, CAT.`id_parent`, CAT.`url`, CAT.`level`, C.`name`")
		...
		if(count($res)>0)
		{
			foreach($res as $ms)
			{
				...
				$url = $this->router->build_url('categorie_lang', array('categorie_url' => $url, 'lang' => $this->mlangs->lang_code));
				...
				if($ms['level']>1)
				{
					if(isset($this->temp_nav_array[$ms['id_parent']]))
					{
						...
						array_push($tmp, $ms + array('categorie_url' => $url));
						$this->temp_nav_array[$ms[self::ID_CAT]] = $tmp;
					}
					else
					{
						$this->temp_nav_array[$ms[self::ID_CAT]] = array($ms + array('categorie_url' => $url));
					}
				}
				else
				{
			...
			foreach($this->temp_nav_array as $key => $ms)
			{
				foreach($ms as $ar)
				{
					$nav_array[$key][] = array($ar['categorie_url'], $ar['name']);
				}
	...
	public function get_cart_products(array $cart)
	{
		if(count($cart)>0)
		{
			...
			$query = $this->get_cart_products_query($pr_id);
			$products_temp_array = $query->get()->result_array();
			$products_array = $this->build_cart_products_data($products_temp_array);
			
			$query = $this->get_cart_products_prices_query($price_id);
			$prices_temp_array = $query->get()->result_array();
	...
	protected function get_cart_products_query(array $products_array)
	{
		...
		$query = $this->db
				->select("A.`".self::ID_PR."` AS ID, A.`sku`, A.`url_key`, A.`status`, A.`in_stock`, A.`bestseller`, A.`sale`, A.`new`, B.`name`, B.`short_description`, B.`full_description`, B.`seo_title`, B.`seo_description`, B.`seo_keywords`, IMG.`image`, IMG.`".self::ID_PR_ALB."` AS ALB_ID")
				->from("`".self::PR."` AS A")
				->join(	"`".self::PR_DESC."` AS B",
						"B.`".self::ID_PR."` = A.`".self::ID_PR."` && B.`".self::ID_LANGS."` = '".$this->mlangs->id_langs."'",
						"LEFT")
	...
	protected function build_cart_products_attr_opt_data(array $attr_array, array $opt_array)
	{
		...
		{
			$query = $this->get_cart_products_attr_opt_query($attr_array, $opt_array);
			$temp_array = $query->get()->result_array();
			foreach($temp_array as $ms)
			{
