...
class Products extends AG_Controller
{
	...
	public function get_categorie_products()
	{
		if($categories_id = $this->variables->get_vars('categorie_id'))
		{
			$this->build_categorie_products_temptate_js();
			$this->load->model('catalogue/mproducts');
			$products_array = $this->mproducts->get_categories_products_collection($categories_id);
			$this->build_categorie_products_template_blocks($products_array);
		}
	}
	
	public function build_categorie_products_temptate_js()
	{
		$this->template->add_js('jquery.gbc_products_short', 'modules_js/catalogue/products');
	}
	
	public function build_categorie_products_template_blocks($products_array)
	{
