...
class Mwarehouses_products extends AG_Model
{
	...
	public function get_wh_products_grid($wh_id)
	{
		$this->load->library('grid');
		$this->grid->_init_grid('wh_products_grid_'.$wh_id, array('limit' => 50, 'url' => set_url('*/*/actions/id/' . $wh_id)), TRUE);

		$this->grid->db->select("A.`" . self::ID_PR . "` AS ID, A.`sku`, B.`name`, A.`status`, C.`qty`")
					   ->from("`" . self::WH_PR . "` AS C")
					   ->join("`" . self::PR . "` AS A", "A.`" . self::ID_PR . "` = C.`" . self::ID_PR . "`", "INNER")
					   ->join("`" . self::PR_DESC . "` AS B", "B.`" . self::ID_PR . "` = A.`" . self::ID_PR . "` && B.`" . self::ID_LANGS . "` = " . $this->id_langs, "LEFT")
	...
	public function get_warehouses_all_pr_grid()
	{
		...
		$this->grid->db
			->select("A.`".self::ID_WH_PR."` AS ID, A.`".self::ID_PR."` AS PR_ID, A.`sku`, C.`name`, GROUP_CONCAT(CONCAT(B.`alias`,' : ', A.`qty`) ORDER BY(B.`sort`) SEPARATOR '<br>') AS WH_QTY")
			->from("`".self::WH_PR."` AS A")
			->join("`".self::WH."` AS B",
					"B.`".self::ID_USERS."` = '".$this->id_users."' && B.`".self::ID_WH."` = A.`".self::ID_WH."`",
					"INNER")
	...
	public function get_print_wh_pr($wh_id)
	{
		$this->db
			->select("A.`".self::ID_PR."` AS PR_ID, A.`sku`, C.`name`, A.`qty`")
			->from("`".self::WH_PR."` AS A")
			->join("`".self::PR_DESC."` AS C",
					"C.`".self::ID_PR."` = A.`".self::ID_PR."` && C.`".self::ID_LANGS."` = '".$this->id_langs."'",
					"LEFT")
			->where("A.`".self::ID_WH."`", $wh_id);
		if($grid_sess_data = $this->session->get_data('GRID_'))
		{
	...
	{
		$array = array();
		$this->db->select("A.`".self::ID_WH."`, A.`alias`, B.qty")
				->from("`".self::WH."` AS A")
	...
	public function edit_wh_pr_qty($wh_id, $pr_id, $sku, $qty)
	{
		...
		$this->sphinx_update_pr_qty($wh_id, $pr_id, $qty);
	}

	public function sphinx_update_pr_qty($wh_id, $pr_id, $qty)
	{
		$this->db->select("`".self::ID_WH."`, `i_s_wh`")
			 ->from("`".self::WH."`")
	...
	public function not_in_wh_pr_grid($wh_id)
	{
		...
		$this->grid->db
			->select("A.`".self::ID_PR."` AS ID, A.`sku`, A.`status`, B.`name`, C.`qty`")
			->from("`".self::PR."` AS A")
			->join("`".self::PR_DESC."` AS B",
					"B.`".self::ID_PR."` = A.`".self::ID_PR."` && B.`".self::ID_LANGS."` = '".$this->id_langs."'",
					"LEFT")
			->join("`".self::WH_PR."` AS C",
					"C.`".self::ID_WH."` = '".$wh_id."' && C.`".self::ID_PR."` = A.`".self::ID_PR."`",
					"LEFT")
			->where("A.`".self::ID_USERS."`", $this->id_users)->where("C.`qty` IS NULL", NULL, FALSE);
	...
	public function save_add_pr_qty($wh_id, $pr_id)
	{
		if(($POST = $this->input->post('warehouse_product')) && isset($POST['add_qty']) && ($add_qty = intval($POST['add_qty']))>0)
		{
			...
			$this->db->select("`".self::ID_WH_PR."`, `sku`, `qty`")
				->from("`".self::WH_PR."`")
				->where("`".self::ID_WH."`", $wh_id)->where("`".self::ID_PR."`", $pr_id)->limit(1);
			$wh_pr = $this->db->get()->row_array();
			if(count($wh_pr) == 0) return FALSE;
			
			$this->load->model('warehouse/mwarehouses_logs');
			$this->db->trans_start();
			$qty = $wh_pr['qty']+$add_qty;
			$this->sql_add_data(array('qty' => ($qty)))->sql_save(self::WH_PR, $wh_pr[self::ID_WH_PR]);
	...
	public function edit_transferred_products($wh_id, $products)
	{
		$this->load->model('warehouse/mwarehouses');
		if(!$this->mwarehouses->check_isset_wh($wh_id)) return FALSE;
		$sphinx_update = array();
		foreach($products as $ms)
		{
			$this->db->select("`".self::ID_WH_PR."`, `sku`, `qty`")
					 ->from("`".self::WH_PR."`")
					 ->where("`".self::ID_WH."`", $wh_id)->where("`".self::ID_PR."`", $ms[self::ID_PR])->limit(1);
			$wh_pr = $this->db->get()->row_array();
			$qty = $wh_pr['qty']-$ms['qty'];
			$this->sql_add_data(array('qty' => $qty))->sql_save(self::WH_PR, $wh_pr[self::ID_WH_PR]);
			$sphinx_update[$ms[self::ID_PR]] = array('qty' => $qty);
		}
		$this->sphinx_update_pr_qty_array($wh_id, $sphinx_update);
		return TRUE;
	}

	public function add_transferred_products($wh_id, $products)
	{
		$this->load->model('warehouse/mwarehouses');
		if(!$this->mwarehouses->check_isset_wh($wh_id)) return FALSE;
		$sphinx_update = array();
		foreach($products as $ms)
		{
			...
			{
				$qty = $ms['qty'];
				$this->sql_add_data(array(self::ID_WH => $wh_id, self::ID_PR => $ms[self::ID_PR], 'sku' => $ms['sku'], 'qty' => $qty))->sql_save(self::WH_PR);
			}
			else
			{
				$qty = $wh_pr['qty']+$ms['qty'];
	...
	public function save_reject_pr_qty($wh_id, $pr_id)
	{
		if(($POST = $this->input->post('warehouse_product')) && isset($POST['reject_qty']) && ($reject_qty = intval($POST['reject_qty']))>0)
		{
			...
			$this->load->model('warehouse/mwarehouses_logs');
			$this->db->trans_start();
			$qty = $wh_pr['qty'] - $reject_qty;
			$this->sql_add_data(array('qty' => $qty))->sql_save(self::WH_PR, $wh_pr[self::ID_WH_PR]);
			$this->mwarehouses_logs->create_reject_pr_log($wh_id, $pr_id, $wh_pr['sku'], $qty, $wh_pr['qty'], $POST['comment']);
			$this->db->trans_complete();	
			if($this->db->trans_status())
			{
				$this->sphinx_update_pr_qty($wh_id, $pr_id, $qty);
	...
	public function delete_pr($wh_id, $pr_id)
	{
		$this->db->select("`qty`, `sku`")
				->from("`".self::WH_PR."`")
				->where("`".self::ID_WH."`", $wh_id)->where("`".self::ID_PR."`", $pr_id)->limit(1);
		if(count($result = $this->db->get()->row_array()) > 0)
		{
			if($result['qty'] != 0) return FALSE;
			$this->db->where("`".self::ID_WH."`", $wh_id)->where("`".self::ID_PR."`", $pr_id)->limit(1)->delete("`".self::WH_PR."`");

			$this->load->model('warehouse/mwarehouses_logs');
			$this->mwarehouses_logs->create_delete_pr_log($wh_id, $pr_id, $result['sku']);
	...
	public function get_create_sale_products($wh_id)
	{
		...
		if(count($cart_array = $this->cart->contents())>0)
		{
			...
			foreach($cart_array as $key => $ms)
			{
				...
				{
					$product_array[$key] = array('i' => $i, 'rowid' => $ms['rowid'], 'PR_ID' => $cart_products_array[$ms['id']]['PR_ID'], 'sku' => $cart_products_array[$ms['id']]['sku'], 'name' => $cart_products_array[$ms['id']]['name'], 'qty' => $ms['qty'], 'price' => $ms['price']);
					$i++;
	...
	public function create_wh_products_sale($wh_id, $products_array)
	{
		$pr_id = array();
		$update_array = array();
		foreach($products_array as $ms)
		{
			$pr_id[$ms[self::ID_PR]] = $ms[self::ID_PR];
		}
		$this->db->select("*")
			->from("`".self::WH_PR."`")
			->where("`".self::ID_WH."`", $wh_id)->where_in("`".self::ID_PR."`", $pr_id);
		$result = $this->db->get()->result_array();
		$wh_pr_array = array();
		foreach($result as $ms)
		{
			$wh_pr_array[$ms[self::ID_PR]] = $ms;
		}
		
		foreach($products_array as $ms)
		{
			if(isset($update_array[$ms[self::ID_PR]])) $update_array[$ms[self::ID_PR]]['qty'] -= $ms['qty'] * $ms['real_qty'];
			else $update_array[$ms[self::ID_PR]] = array(self::ID_WH_PR => $wh_pr_array[$ms[self::ID_PR]][self::ID_WH_PR], 'qty' => $wh_pr_array[$ms[self::ID_PR]]['qty'] - ($ms['qty'] * $ms['real_qty']));
		}
		
		foreach($update_array as $ms)
		{
			$this->sql_add_data(array('qty' => $ms['qty']))->sql_save(self::WH_PR, $ms[self::ID_WH_PR]);
		}
		$this->sphinx_update_pr_qty_array($wh_id, $update_array);
		return TRUE;
	}
	
	public function cancel_wh_products_sale($wh_id, $products_array)
	{
		$pr_id = array();
		$update_array = array();
		foreach($products_array as $ms)
