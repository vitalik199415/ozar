...
if(isset($PRD_block_id) && isset($PRD_array))
{
	...
	if(isset($PRD_array['prices_array']) && count($PRD_array['prices_array'])>0)
	{
		foreach($PRD_array['prices_array'] as $ms)
		{
			...
			{
			?>
				prices_array[<?=$ms['PRICE_ID']?>] = '<?=$ms['special_price_rate'] + 0?>';
			<?
			}
			else
			{
			?>
				prices_array[<?=$ms['PRICE_ID']?>] = '<?=$ms['price_rate'] + 0?>';
			<?
			}
			?>
	...
	wh_sale_object.gbc_wh_sales('init_view_product', <?=$PRD_ID?>, prices_array, prices_attributes, <?=$sale_id?>);
	</script>
<?php
}
?>