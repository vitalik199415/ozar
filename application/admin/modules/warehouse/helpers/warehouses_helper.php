...
function helper_wh_actions_form_build($data, $wh_id)
{
	...
	$CI = & get_instance();
	$CI->load->library('form');
	$CI->form->_init('Действия', $form_id, '');
	...
	$CI->form->add_button(
		array(
		'name' => 'Список продаж',
		'href' => set_url('warehouse/warehouses_products/wh_sales_grid/wh_id/'.$wh_id)
	));
	$CI->form->add_button(
		array(
	...
	$init_show_product = "
	<script>
		$('#".$form_id."').find('#wh_products_grid_".$wh_id."').gbc_show_product();
	</script>
	";
	$CI->form->group('p_b')->add_html_to($lid, $data['wh_products'].$init_show_product);
...
function helper_warehouse_sales_grid_build(Grid $grid, $wh_id)
{
	$grid->add_button('Создать продажу', set_url('*/warehouses_products/create_sale/wh_id/'.$wh_id),
		array(
			'rel' => 'add',
			'class' => 'addButton',
		));
		 ...
		 array(
			 ...
			 'actions'	 => array(
				 array(
					 'type' 			=> 'link',
					 'html' 			=> '',
					 'href' 			=> set_url(array('*', 'warehouses_products', 'wh_sales_view', 'wh_id', '$2', 'sale_id', '$1')),
...
function helper_wh_shops_grid_build(Grid $grid, $data)
{
