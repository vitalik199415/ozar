<?php
class Block_additionally_footer extends AG_Controller
{
	const BACK = '*/*/';
	const N = 4;
	protected $block_id = 1;
	
	function __construct()
	{
		parent::__construct();
		$this->template->add_title('Дополнительный блок - Footer');
		$this->template->add_navigation('Дополнительный блок')->add_navigation('Footer', set_url(self::BACK));
	}
	...
	public function add()
	{
		$this->template->add_title('Добавление блока в Footer');
		$this->template->add_navigation('Добавление блока в Footer');
		$this->load->model('block_additionally/mblock_additionally');
	...
	{
		$this->template->add_title('Редактирование блока в Footer');
		$this->template->add_navigation('Редактирование блока в Footer');		
		$this->load->model('block_additionally/mblock_additionally');
		...
		if(isset($URI['id']) && ($ID = intval($URI['id']))>0)
		{
			...
			{
				$this->messages->add_error_message('Возникли ошибки генерации редактирования блока в Footer');
	...
	public function save()
	{
		if(isset($_POST))
		{
			...
			if(isset($URI['id']) && intval($URI['id'])>0)
			{
				...
				{
					$this->messages->add_success_message('Oбъект блока Footer успешно отредактирован!');
					$this->_redirect(set_url(self::BACK));
				}
				else
				{
					$this->messages->add_error_message('Возникли ошибки при редактировании объекта блока Footer!');
					$this->_redirect(set_url(self::BACK));
				}
			...
			else
			{
				...
				{
					$this->messages->add_success_message('Oбъект блока Footer успешно добавлен!');
					$this->_redirect(set_url(self::BACK));
					if (isset($_GET['return']))
					{
						$this->_redirect(set_url(self::BACK.'edit/id/'.$ID));
				...
				{
					$this->messages->add_error_message('Возникли ошибки при добавлении объекта Footer!');
					$this->_redirect(set_url(self::BACK));
				}
			}
		}
	...
	public function delete()
	{
		...
		if(isset($URI['id']) && intval($URI['id'])>0)
		{
			...
			{
				$this->messages->add_success_message('Oбъект блока Footer успешно удален!');
				$this->_redirect(set_url(self::BACK));
			}
			else
			{
				$this->messages->add_error_message('Oбъект блока Footer с ID = '.$id.' не существует, или произошла ошибка при удалении!');
				$this->_redirect(set_url(self::BACK));
			}
		}
		else
