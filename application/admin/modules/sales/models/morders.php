...
class Morders extends AG_Model
{
	...
	public function render_orders_collection_grid()
	{
		...
		$this->grid->db
				->select("A.`".self::ID_ORD."` AS ID, A.`orders_number` AS `orders_number`, A.`orders_state`, CONCAT(A.`total`, '(<span class=\'label\'>', A.`discount`, '</span>) ', A.`base_currency_name`) AS total, A.`id_m_c_currency`,A.`currency_name`, A.`currency_rate`, A.`create_date`, A.`update_date`, B.`name`")
				->from("`".self::ORD."` AS A")
	...
	public function render_order_products_grid($ord_id = 0, $ajax = FALSE)
	{
        ...
        if($ord_id > 0) {
            $this->load->model('catalogue/mdiscounts');
            $res = $this->mdiscounts->get_discount($product_array, $ord_id);
            if($res['result'] == 2) {
                $product_array = $res['products'];
            }
        }
	...
	public function prepare_order_view_products_data($ord_id = 0)
	{
		...
		if($ord_id)
		{
			...
			{
				$this->db->select("A.`".self::ID_PR."` AS PR_ID, A.`".self::ID_ORD_PR."` AS `ORD_PR_ID`, A.`sku`, A.`name`, CONCAT('<span class=\'label\'>', A.`price_name`, '</span>', ' ', ROUND(A.`price`, 2)) AS `price`, IF(A.`real_qty` <> 1, CONCAT(A.`qty`,'(<span class=\'label\'>',A.`qty`*A.`real_qty`, '</span>)'), A.`qty`) AS qty_str, A.`qty`, A.`real_qty`,IF(A.`discount` <> NULL, CONCAT(ROUND(A.`total`, 2),'(<span class=\'label\'>',ROUND(A.`discount`, 2), '</span>)'), ROUND(A.`total`, 2)) AS total, GROUP_CONCAT(CONCAT(B.`attributes_name`, ' : ', B.`attributes_options_name`) SEPARATOR '<BR>') AS attributes")
						 ->from("`".self::ORD_PR."` AS A")
						 ->join(	"`".self::ORD_PR_ATTR."` AS B",
							 "B.`".self::ID_ORD_PR."` = A.`".self::ID_ORD_PR."`",
							 "LEFT")
	...
	public function view_order($ord_id)
	{
		...
		//$this->load->model('catalogue/mdiscounts');
		//$order['order']['discount'] = $this->mdiscounts->get_discount($order['order']['subtotal']);
		$order_data += $order;
	...
	public function get_order_sum($ord_id)
	{
		...
		if(count($order)>0)
		{
			...
			if(($discount = floatval($order['discount'])) > 0)
			{
				...
				$total_rate = ($total - $discount) * $order['currency_rate'];
			}
			else
			{
				$discount = $this->mdiscounts->get_discount($ord_id)['discount'];
				$subtotal = $order['subtotal'];
				$total = $order['total'];
            ...
            $discount = $discount * $order['currency_rate'];
...
//ORDER PRODUCTS ACTIONS
	...
	public function get_temp_order_products_sum($ord_id = 0)
	{
        ...
        if($ord_id > 0) {
            $products_array = $this->prepare_order_view_products_data($ord_id);
            $discount = $this->mdiscounts->get_discount($products_array, $ord_id);
        } else {
            $discount = $this->mdiscounts->get_discount($total_sum);
        }
		/*if($discount = $this->_get_order_discount_temp($ord_id))*/
		if($discount['result'] == 1 || $discount['result'] == 2)
		{
			$subtotal = $total_sum;
			$total = $total_sum;

			$subtotal_rate = $total_sum * $currency['rate'];
			$total_rate = ($total_sum - $discount['discount']) * $currency['rate'];
		}
		else
		{
			$subtotal = $total_sum;
        ...
        $discount = $discount['discount'] * $currency['rate'];
...
//ORDER SAVE
	...
	public function save($ord_id = FALSE)
	{
		...
		if($ord_id)
		{
            ...
            if($this->_get_order_discount_temp($ord_id)) {
                $discount = $this->_get_order_discount_temp($ord_id);
            } else {
                $discount = $this->mdiscounts->get_discount($ord_id);
            }
            //$discount = floatval($ORDER_POST['discount']);
			...
			$order_data['total'] = $order_total['subtotal'] - $discount;
		...
		else
		{
            ...
            if($ORDER_POST['discount'] == 0) {
                $discount = $this->mdiscounts->get_discount($order_total['subtotal']);
            } else {
                $discount = floatval($ORDER_POST['discount']);
            }
