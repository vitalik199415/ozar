<?php
class Products_similar extends AG_Controller
{
	function __construct()
	{
		parent::__construct();
		$this->template->add_title('Похожые продукты');
		$this->template->add_navigation('Каталог продукции')->add_navigation('Похожые продукты', set_url('*/*'));
	...
	public function index()
	{
		...
		$this->load->model('catalogue/mproducts_similar');
		$this->mproducts_similar->render_product_similar_grid();
		if(!$this->input->post('ajax')) $this->template->add_template('catalogue/products/show_product_js', array('show_products_block_id' => 'products_similar_grid'));
	}
	...
	public function get_similar()
	{
		...
		if(isset($URI['id']) && intval($URI['id'])>0)
		{
			...
			$similar_products = $this->mproducts_similar->get_similar_products($ID);
			if(count($similar_products) == 0)
			{
				$this->_redirect(set_url('*/*/add_similar/id/'.$ID));
				//$this->add_related();
			}
			else
			{
				if(!$this->mproducts_similar->get_similar($ID))
				{
	...
	public function add_similar()
	{
		...
		if(isset($URI['id']) && intval($URI['id'])>0)
		{
			...
			if(!$this->mproducts_similar->add_similar($ID))
			{
				$this->messages->add_error_message('Возникли ошибки генерации добавления товаров!');
				$this->_redirect(set_url('*/*'));
			}
	...
	public function save_similar()
	{
		if($this->input->post('products_similar_checkbox'))
		{
			...
			{
				$id = intval($URI['id']);
				if($this->mproducts_similar->save_similar($id))
				{
					$this->messages->add_success_message('Продукты успешно добавлены в сопутствующие!');
					$this->_redirect(set_url('*/*/get_similar/id/'.$id));
				}
				else
				{
	...
	{ 	
		...
		$similar = $this->uri->uri_to_assoc(6);
		
		if (isset($parent['id']) && ($id_parent = intval($parent['id']))>0 )
		{ 
			if (isset($similar['pr_id']) && ($id_similar = intval($similar['pr_id']))>0)
			{
				if($this->mproducts_save->check_isset_pr($id_similar))
				{
					$this->mproducts_similar->delete_similar($id_parent, $id_similar);
					$this->messages->add_success_message('Продукт успешно удален из похожых!');
					$this->_redirect(set_url('*/*/get_similar/id/'.$id_parent));
			...
			{
				$this->mproducts_similar->delete_similar($id_parent);
				$this->messages->add_success_message('Продукт успешно удален из похожых!');
				$this->_redirect(set_url('*/*/get_similar/id/'.$id_parent));
			}
	...
	public function get_ajax_add_similar()
	{
		$URI = $this->uri->uri_to_assoc(4);
		if(isset($URI['id']) && ($id = intval($URI['id']))>0)
		{
			$this->load->model('catalogue/mproducts_similar');
			echo $this->mproducts_similar->add_similar_grid_build($id);
		}
	}
	
	public function get_ajax_get_similar()
	{
		$URI = $this->uri->uri_to_assoc(4);
		if(isset($URI['id']) && ($id = intval($URI['id']))>0)
		{
			$this->load->model('catalogue/mproducts_similar');
			echo $this->mproducts_similar->get_similar_grid_build($id);
		}
	}
