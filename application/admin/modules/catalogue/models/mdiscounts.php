...
class Mdiscounts extends AG_Model
{
    ...
    /*public function get_discount($order_summ) {
        $discount = $this->db ->select('*')
            ->from('`'.self::DISC.'`')
            ->where('`sum_from` < '.$order_summ)->where('`active`', 1)
    ...
    public function get_discount($ord_data = FALSE, $ord_id = FALSE) {
        if($ord_id && is_array($ord_data)) {
            $this->load->model('catalogue/mdiscount_coupons');
            return $this->mdiscount_coupons->check_coupon_discount($ord_data, $ord_id);
        } else
        if(is_float($ord_data)) {
            return $this->check_discount($ord_data);
        }
    }

    public function check_discount($total) {
        $discount = $this->db ->select('*')
            ->from('`'.self::DISC.'`')
            ->where('`sum_from` < '.$total)
        ...
        if(count($discount) > 0) {
            if($discount['type_discounts'] == 0) {
                ...
                $disc = $total * $discount['discount_percent'] / 100;
            }
        }
