...
class Mpermissions_modules extends AG_Model {
    ...
    const   PERM_TYPES          = 'm_permissions_types';
    const   ID_PERM_TYPES       = 'id_m_permissions_types';
    const   PERM_TYPES_DESC     = 'm_permissions_types_description';
    const   ID_PERM_TYPES_DESC  = 'id_m_permissions_types_description';
...
//        $this->grid->update_grid_data('is_start', array('0' => 'Нет', '1' => 'Да'));
    ...
    public function add() {
        $this->db
             ->select("`langs`.`id_langs`, `langs`.`name`")
             ->from("`langs`")
             ->join("`users_langs`", "`users_langs`.`id_langs` = `langs`.id_langs", "INNER")
             ->where("`users_langs`.`id_users`", $this->id_users)
             ->where('`langs`.`active`', 1);
    ...
    public function edit($id){
        ...
        if(count($main)>0)
        {
            ...
            $permissions = $this->db->select(self::ID_PERM_TYPES.' as ID, alias')
