<?php
class Mm_catalogue_settings extends AG_Model
{
	const MID				= '3';
	...
	{
		$this->load->helper('m_catalogue/m_catalogue_settings_helper');
		$data['img_settings']['img_settings'] = $this->get_m_catalogue_settings(TRUE);
		helper_m_catalogue_settings_form_build($data);
	}
	
	public function get_m_catalogue_settings($return_key_value = FALSE, $prefix_key = FALSE)
	{	
		$query = $this->db
	...
	public function save()
	{
		...
		$settings = $this->get_m_catalogue_settings();
		$this->db->trans_start();
