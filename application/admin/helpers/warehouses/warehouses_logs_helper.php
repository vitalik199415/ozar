<?php
function helper_logs_grid_build($grid, $wh_array)
{
	$grid->add_button('Продажи', set_url('*/warehouses_logs/sales_logs'));
	$grid->add_button('Перенос', set_url('*/warehouses_logs/transfers_logs'));
	...
	$grid->add_button('Продукт добавлен', set_url('*/warehouses_logs/add_pr_logs'));
	$grid->add_button('Продукт удален', set_url('*/warehouses_logs/delete_pr_logs'));
	...
	$grid->add_column(
		array
			(
				'index'		 => 'type',
				'type'		 => 'select',
				'options'	 => array('' => '') + mwarehouses_logs::get_log_types(),
				'tdwidth'	 => '14%',
				'filter'	 => true
			), 'Тип');
...
function helper_sales_logs_grid_build($grid, $wh_array)
{
	$grid->add_button('Все логи', set_url('*/warehouses_logs'));
	$grid->add_button('Переносы', set_url('*/warehouses_logs/transfers_logs'));
	$grid->add_button('Количество добавлено', set_url('*/warehouses_logs/edit_pr_logs'));
	$grid->add_button('Списан', set_url('*/warehouses_logs/reject_pr_logs'));
	$grid->add_button('Продукт добавлен', set_url('*/warehouses_logs/add_pr_logs'));
	$grid->add_button('Продукт удален', set_url('*/warehouses_logs/delete_pr_logs'));
...
function helper_transfers_logs_grid_build($grid, $wh_array)
{
	$grid->add_button('Все логи', set_url('*/warehouses_logs'));
	$grid->add_button('Продажи', set_url('*/warehouses_logs/sales_logs'));
	$grid->add_button('Количество добавлено', set_url('*/warehouses_logs/edit_pr_logs'));
	...
	$grid->add_column(
		array
			(
				'index'		 => mwarehouses_logs::ID_WH.'_to',
				'type'		 => 'select',
				'searchtable' => 'B',
				'options'	 => array('' => '') + $wh_array,
				'tdwidth'	 => '11%',
...
function helper_sales_grid_build($grid, $wh_id, $wh_alias)
{
	$grid->add_button('Склад '.$wh_alias, set_url('*/warehouses/wh_actions/wh_id/'.$wh_id));
	$grid->add_button('Создать продажу(Склад '.$wh_alias.')', set_url('*/warehouses_products/create_sale/wh_id/'.$wh_id));
