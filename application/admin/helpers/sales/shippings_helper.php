<?php
function helper_shippings_grid_build($grid)
{
...
function helper_shipping_view($data)
{
	...
	$CI->form->add_button(
		...
		'href' => set_url('*/invoices/view/inv_id/'.$data['invoice']['id_m_orders_invoices'])
	));
	if($data['shipping']['shippings_status'] != 'CN' && $data['invoice']['invoices_status'] != 'C')
	{
		$CI->form->add_button(
			...
			'href' => set_url('*/*/cancel_shipping/shp_id/'.$data['shipping']['id_m_orders_shippings']),
			'options' => array(
	...
	if($data['shipping']['shippings_status'] == 'C')
	{
		$CI->form->add_button(
			...
			'href' => set_url('*/shippings/send_shipping_mail/shp_id/'.$data['shipping']['id_m_orders_shippings'])
		));
	}
	...
	$CI->form->group('o_d')->add_object_to($lid,
		'html',
		'<div align="center" style="margin:0 0 10px 0;"><a href="'.set_url('*/orders/ajax_show_products_with_photo/ord_id/'.$data['order']['id_m_orders']).'" style="font-size:18px;" id="show_products_with_photo">Посмотреть продукты заказа с фото</a></div>'
	);
