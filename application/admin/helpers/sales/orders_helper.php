<?php
function helper_get_order_statuses()
{
	return array('N' => 'Новый', 'P' => 'В процессе', 'I' => 'Ожидание оплаты', 'IC' => 'Оплочен', 'S' => 'Процесс отправки', 'C' => 'Завершен', 'CN' => 'Отменен', 'H' => 'Временно заморожен');
}

function helper_orders_grid_build($grid)
{
...
//WH products
function helper_orders_wh_products_grid_build($grid, $ord_id)
{
	$CI = & get_instance();
	$URI = $CI->uri->uri_to_assoc(4);
	
	$grid->add_button('Отменить изменения', set_url('*/*/ajax_unset_products_session_data/ord_id/'.$ord_id), 
		array(
			'rel' => 'add',
			'class' => 'addButton',
			'id' => 'unset_products_session_data'
		));
		...
		array(
			...
			'actions'	 => array(
				...
				array(
					...
					'href_values' 	=> array('ORD_PR_ID'),
					'options'		=> array('class' => 'icon_delete delete_product_qty', 'title' => 'Редактировать количество')
				)
			)
		), 'Действие');
}
...
function helper_orders_products_grid_build($grid, $ord_id)
{
	$CI = & get_instance();
	$URI = $CI->uri->uri_to_assoc(4);
...
function helper_not_edited_orders_products_grid_build($grid, $ord_id)
{
	$CI = & get_instance();
	$URI = $CI->uri->uri_to_assoc(4);
...
function helper_shop_products_grid_build($grid, $ord_id = 0)
{
	$grid->add_column(
		array(
			'index'		 => 'sku',
...
function helper_orders_form_view($data, $id = '')
{
	$form_id = 'orders_view_form';
	$CI = & get_instance();
	$CI->load->library('form');
	$CI->form->_init('Просмотр заказа', $form_id, set_url('*/*/view_save/ord_id/'.$id));
	...
	if($data['order']['orders_state'] != 'CN' && ($data['invoice'] === FALSE || ($data['invoice']['invoices_status'] == 'N' || $data['invoice']['invoices_status'] == 'P')))
	{
		$CI->form->add_button(
			array(
			'name' => 'Отменить заказ',
			'href' => set_url('*/*/cancel_order/ord_id/'.$id)
		));
	}
	if($data['invoice'] !== FALSE)
	{
		$CI->form->add_button(
			array(
			'name' => 'Инвойс '.$data['invoice']['invoices_number'],
			'href' => set_url('*/invoices/view_invoice/inv_id/'.$data['invoice']['id_m_orders_invoices'])
		));
	}
	if(($data['order']['orders_state'] != 'CN' && $data['invoice'] !== FALSE && $data['invoice']['invoices_status'] == 'C') && $data['shipping'] === FALSE)
	{
		$CI->form->add_button(
			array(
			'name' => 'Создать отправку',
			'href' => set_url('*/shippings/create_shipping/ord_id/'.$id)
		));
	}
	...
	if($data['order']['orders_state'] != 'CN' && $data['invoice'] === FALSE)
	{
		$CI->form->add_button(
			array(
			'name' => 'Создать инвойс',
			'href' => set_url('*/invoices/create_invoice/ord_id/'.$id)
		));
	}
	$CI->form->add_button(
		array(
		'name' => 'Печать заказа',
		'href' => set_url('*/*/print_order/ord_id/'.$id),
		'options' => array(
			'target' => '_blank'
		)
	));
	if($data['invoice'] === FALSE && ($data['order']['orders_state'] == 'N'))
	{
		$CI->form->add_button(
			array(
	...
	$CI->form->group('m_b')->add_view_to($lid, 'sales/orders/view_order_customer', $data);
	...
	$init_show_product = "
	<script>
		$('#".$form_id."').find('#orders_products_grid').gbc_show_product();
	</script>
	";
	$CI->form->group('m_b')->add_html_to($lid, $data['products'].$init_show_product);
	...
	$CI->form->group('m_b')->add_object_to($lid,
		'html',
		'<div align="center" style="margin:0 0 10px 0;"><a href="'.set_url('*/*/ajax_show_products_with_photo/ord_id/'.$id).'" style="font-size:18px;" id="show_products_with_photo">Посмотреть продукты заказа с фото</a></div>'
	);
	$CI->form->group('m_b')->add_object_to($lid,
		'select', 
		'order[status]',
	...
	$CI->form->group('m_b')->add_object_to($lid,
		...
		'Валюта заказа :',
		array(
			'options'	=> $data['users_currency']['currency'],
			'option'	=> array('readonly' => NULL, 'disabled' => NULL)
		)
	);
	$CI->form->group('m_b')->add_object_to($lid,
	...
	if(isset($data['order']['users_payment_methods']))
	{
		$CI->form->group('m_b')->add_object_to($lid,
			'select',
			...
			'Метод оплаты (*):',
			array(
				'options'	=> $data['order']['users_payment_methods'],
				'option' => $select_disabled
			)
		);
	}
	if(isset($data['order']['users_shipping_methods']))
	{
		$CI->form->group('m_b')->add_object_to($lid,
			'select',
			...
			'Метод доставки (*):',
			array(
				'options'	=> $data['order']['users_shipping_methods'],
				'option' => $select_disabled
			)
		);
	}
	...
	$CI->form->group('a_b')->add_view('sales/orders/view_order_js', array('order_id' => $id));
